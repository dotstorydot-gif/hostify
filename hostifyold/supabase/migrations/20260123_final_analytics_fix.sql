-- 1. Add guest_count to Bookings (for Guest Type Analytics)
ALTER TABLE bookings ADD COLUMN IF NOT EXISTS guest_count INTEGER DEFAULT 1;

-- 2. Clean up old external bookings to force re-sync with valid Revenue/GuestCount
DELETE FROM bookings WHERE external_booking_id IS NOT NULL;

-- 3. Enforce 15% Management Fee
...
-- 4. Update Analytics Function (FINAL VERSION)
...
BEGIN
  ...
  -- Financials
  SELECT 
    COALESCE(SUM(CASE WHEN b.total_price > 0 THEN b.total_price ELSE (b.nights * p.price_per_night) END), 0),
    COUNT(*),
  ...
  -- Management Fees (15%)
  SELECT COALESCE(SUM(
    (CASE WHEN b.total_price > 0 THEN b.total_price ELSE (b.nights * p.price_per_night) END) 
  ...
  -- Nationality (Corrected: using user_profiles or fallback)
  ...
  -- Guest Type (Derived from guest_count)
  ...
  FROM (
    SELECT DATE_TRUNC('month', b.check_in) as month_date, SUM(CASE WHEN b.total_price > 0 THEN b.total_price ELSE (b.nights * p.price_per_night) END) as revenue
    FROM bookings b JOIN properties p ON b.property_id = p.id
    WHERE p.landlord_id = p_landlord_id AND b.check_in >= v_start AND b.check_in <= v_end
